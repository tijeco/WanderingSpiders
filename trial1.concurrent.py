...
def reproduce(daddy): # I will certainly need to retool this for multiple toxins
    ...
    return babies
def weekly_feeding(spider,spiderCluster,inCricket,t,year,final_spiders):
...
spider_cluster = {"spiders": [sp.spider(1,ld50_low,ld50_high) for i in range(10)]}
# print_year1 = True
def annualFeeding(spiderCluster,inCricket,year):
    ...
    for t in range(52): # consider making into annualFeeding(spider_clster,cricket) function
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(weekly_feeding, spider,spiderCluster,inCricket,t,year,final_spiders) for spider in spiderCluster]
        ...
        # for spider in spiderCluster: # NOTE: this should be able to be paralelized somehow
                # TODO: make function here: weekly_feeding(spider,cricket,week,year,final_spiders_df)
            # weekly_feeding(spider,spiderCluster,inCricket,t,year,final_spiders)
            # for idx, future in enumerate(concurrent.futures.as_completed(futures, timeout=180.0)):
            #     res = future.result()  # This will also raise any exceptions
            #     print("Processed job", idx, "result", res)
